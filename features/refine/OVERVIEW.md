...
- `app/page.tsx` — the single page (`'use client'`). Owns `useRefineStream`; `PromptInput` sits on top, and once a refine starts the result and `WhatChanged` panel render side-by-side at ~60/40 on desktop (`md:grid-cols-[3fr_2fr]`), stacked on mobile. The idle empty-state and error state span full width. Each panel carries an eyebrow label (ROUGH PROMPT / RESULT / WHAT CHANGED) outside its card.
