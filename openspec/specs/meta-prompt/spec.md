...
Defines the refining system prompt (the "meta-prompt") that instructs the model to rewrite a user's rough Claude Code prompt into a Claude-Code-optimised one, and the contract for how that prompt is exported, what refining behaviours it encodes, and how its output is structured.
